...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <select
            ...
            <option value="activo"
                @selected(old('status', $service->status ?? 'activo') == 'activo')>
                Activo
            </option>

            <option value="inactivo"
                @selected(old('status', $service->status ?? '') == 'inactivo')>
                Inactivo
            </option>
...
<div class="mt-6">
    ...
    <textarea
        ...
        class="w-full rounded-2xl bg-[#0c223d] border border-cyan-500/20 text-white placeholder-slate-400 px-5 py-3 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-500 outline-none">{{ old('description', $service->description ?? '') }}</textarea>
